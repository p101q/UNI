...
createPart('hand-base', true, 43.5, -43.5, null,
    [50,155,145], // углы поворота при работе блока
    ...
    [
        createPart('hand', false, 0, 0, null,
            [135,110,145], // углы поворота при работе блока
            [80,70,60], // время, за которое рука на этот угол поворачивает(примерное)
            [60,70,80], // время остановки после отработки угла
            ...
            [
                createPart('hand-laser-press', false, 0, 0, null,
                    [195,145,135], // углы поворота при работе блока
                    [80,70,60], // время, за которое рука на этот угол поворачивает(примерное)
                    [60,70,80], // время остановки после отработки угла
...
createPart('hand-base-reverse', true, -43.5, 43.5, null,
    [295,210,359], // углы поворота при работе блока
    [60,70,60], // время, за которое рука на этот угол поворачивает(примерное)
    [70,70,80], // время остановки после отработки угла
    [340], // углы при неработе блока
    [360], // время, за которое рука на этот угол поворачивает(примерное)
    [
        createPart('hand-reverse', false, 0, 0, null,
            [340,310,335], // углы поворота при работе блока
            [60,70,60], // время, за которое рука на этот угол поворачивает(примерное)
            [70,70,80], // время остановки после отработки угла
            [270], // углы при неработе блока
            [360], // время, за которое рука на этот угол поворачивает(примерное)
            [
                createPart('hand-welder', false, 0, 0, welding,
                    [270,280,295], // углы поворота при работе блока
                    [60,70,60], // время, за которое рука на этот угол поворачивает(примерное)
                    [70,70,80], // время остановки после отработки угла
                    [270], // углы при неработе блока
                    [360], // время, за которое рука на этот угол поворачивает(примерное)
...
createPart('hand-base', true, -43.5, -43.5, null,
    [55,30,105], // углы поворота при работе блока
    ...
    [
        createPart('hand', false, 0, 0, null,
            [120,55,45], // углы поворота при работе блока
            [60,70,80], // время, за которое рука на этот угол поворачивает(примерное)
            [80,70,60], // время остановки после отработки угла
            [90], // углы при неработе блока
            [360], // время, за которое рука на этот угол поворачивает(примерное)
            [
                createPart('hand-screw-driver', false, 0, 0, screwing,
                    [35,-30,-70], // углы поворота при работе блока
                    [60,70,80], // время, за которое рука на этот угол поворачивает(примерное)
                    [80,70,60], // время остановки после отработки угла
...
const hextativeReconstructor = extend(Reconstructor, 'hextative-reconstructor', {
    init() {
        this.super$init();
        for (var i in parts) {
...
hextativeReconstructor.buildType = prov(() => extend(Reconstructor.ReconstructorBuild, hextativeReconstructor, {
    hands: null,
    working: false,
    updateTile() {
