...
createPart('hand-base', true, -35, -35, null,
    [105,65,125], // углы поворота при работе блока
    [70,60,50], // время, за которое рука на этот угол поворачивает(примерное)
    [50,60,70], // время остановки после отработки угла
    [45], // углы при неработе блока
    [360], // время, за которое рука на этот угол поворачивает(примерное)
    [
        createPart('hand', false, 0, 0, null,
            [70,-30,45], // углы поворота при работе блока
            [70,60,50], // время, за которое рука на этот угол поворачивает(примерное)
            [50,60,70], // время остановки после отработки угла
            ...
            [
                createPart('hand-welder', false, 0, 0, welding,
                    [-25,45,-70], // углы поворота при работе блока
                    [70,60,50], // время, за которое рука на этот угол поворачивает(примерное)
                    [50,60,70], // время остановки после отработки угла
                    [135], // углы при неработе блока
                    [360], // время, за которое рука на этот угол поворачивает(примерное)
                    [])
            ])
    ]
...
createPart('hand-base', true, 35, -35, null,
    [55,110,35], // углы поворота при работе блока
    [50,60,70], // время, за которое рука на этот угол поворачивает(примерное)
    [70,60,50], // время остановки после отработки угла
    [135], // углы при неработе блока
    [360], // время, за которое рука на этот угол поворачивает(примерное)
    [
        createPart('hand', false, 0, 0, null,
            [100,140,155], // углы поворота при работе блока
            [50,60,70], // время, за которое рука на этот угол поворачивает(примерное)
            [70,60,50], // время остановки после отработки угла
            [90], // углы при неработе блока
            [360], // время, за которое рука на этот угол поворачивает(примерное)
            [
                createPart('hand-screw-driver', false, 0, 0, screwing,
                    [175,220,245], // углы поворота при работе блока
                    [50,60,70], // время, за которое рука на этот угол поворачивает(примерное)
                    [70,60,50], // время остановки после отработки угла
                    [45], // углы при неработе блока
                    [360], // время, за которое рука на этот угол поворачивает(примерное)
...
const pentativeReconstructor = extend(Reconstructor, 'pentative-reconstructor', {
    init() {
        this.super$init();
        for (var i in parts) {
...
pentativeReconstructor.buildType = prov(() => extend(Reconstructor.ReconstructorBuild, pentativeReconstructor, {
    hands: null,
    working: false,
    updateTile() {
